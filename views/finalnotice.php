           ...
           foreach ($result as $key => $value) {}
              ...
              if ($y =='University') {?>
                <div class="alert alert-info" role="alert">
                 ...
                 <p>The seven days are completed to your Grievance at University level if you not get any responce from them you can send your Grievance to AICTE PORTAL or you can wait.</p>
                 <hr>
                 <form class="form-inline" action="../process/grievanceuni.php" method="post">
                 <input type="hidden" name="caseId" value="<?php echo $value['id']; ?>">
                 <button class="btn btn-info my-2 my-sm-0" type="submit"name="LevelAICTE">Send to AICTE</button>
                 </form>
               </div>
          <?php    } else {?>
            <div class="alert alert-info" role="alert">
             <h4 class="alert-heading text-danger">Important Notice</h4>
             <p>Your Grievance is forwarded to AICTE level.</p>
             <hr>
           </div>
          <?php  }
